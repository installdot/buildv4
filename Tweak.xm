...
//         presentViewController calls, popoverPresentationController source
//         set on every UIAlertController for iPad compatibility.
...
//
//  On iPadOS 13+ the app may have multiple UIWindowScenes.
//  UIApplication.sharedApplication.windows is deprecated in iOS 15 and can
//  return an empty or stale array on some iPad configurations.
//  We walk connected scenes to find the foreground active key window.
// ─────────────────────────────────────────────────────────────────────────────
static UIWindow *keyWindowSafe(void) {
    // iOS 13+ scene-based lookup (preferred)
    if (@available(iOS 13.0, *)) {
        for (UIScene *scene in UIApplication.sharedApplication.connectedScenes) {
            if (scene.activationState != UISceneActivationStateForegroundActive) continue;
            if (![scene isKindOfClass:[UIWindowScene class]]) continue;
            UIWindowScene *ws = (UIWindowScene *)scene;
            // Prefer the key window of the scene
            for (UIWindow *w in ws.windows) {
                if (w.isKeyWindow) return w;
            }
            // Fall back to any visible window in this scene
            for (UIWindow *w in ws.windows) {
                if (!w.isHidden && w.alpha > 0) return w;
            }
        }
        // No foreground-active scene found — try foreground-inactive as fallback
        for (UIScene *scene in UIApplication.sharedApplication.connectedScenes) {
            if (![scene isKindOfClass:[UIWindowScene class]]) continue;
            UIWindowScene *ws = (UIWindowScene *)scene;
            for (UIWindow *w in ws.windows) {
...
- (void)tapHide {
    ...
    // iPad popover anchor — required to prevent crash on iPad
...
//
//  Old code iterated UIApplication.sharedApplication.windows directly.
//  On iPad with iOS 13+ multi-scene this can miss the active scene entirely
//  and return nil, causing every presentViewController call to crash with:
//    "Attempt to present <UIAlertController> on <nil>"
//
//  New code uses keyWindowSafe() which walks UIWindowScene.windows first,
//  then falls back to the legacy API so iOS 12 jailbreaks still work.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// MARK: - Injection  (v10.7 — scene-aware)
// ─────────────────────────────────────────────────────────────────────────────
...
static void injectPanel(void) {
    UIWindow *win = keyWindowSafe();
    if (!win) {
        // Retry once more after a short delay in case the window isn't ready yet
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)),
                       dispatch_get_main_queue(), ^{
            UIWindow *w2 = keyWindowSafe();
            if (!w2) return;
            UIView *root2 = w2.rootViewController.view ?: w2;
            gPanel = [SKPanel new];
