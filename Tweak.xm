...
// ═════════════════════════════════════════════════════════════════════════════
//  UNITOREIOS DEV TOOL
//  • Injects at priority 101 — before Unitoreios +load (priority 65535)
//  • Force offline on/off (persistent across restarts)
//  • Live remainingSeconds from memory, updates every second
//  • +7 days button
//  • Inject fake cached session into all 5 memory guards
...
// ── forward declare Unitoreios internals ──────────────────────────────────────
...
// ── extern globals from Unitoreios.mm (all decrypted at runtime) ──────────────
...
extern NSString * const encodestring;   // compile-time constant "odsugddhwxbc=="
extern NSString        *encodedcode;    // runtime — must equal encodestring
extern NSString        *keyValidationStatus;
extern NSString        *iskey;

// ── runtime-captured statics (can't extern, grabbed via hooks) ────────────────
...
// ── singleton ref (grabbed from startUpdateTimer hook) ────────────────────────
...
// ═════════════════════════════════════════════════════════════════════════════
//  SESSION INJECTION
//  Writes all 5 guards that +paid: checks simultaneously:
//    1. iskey               (RAM NSString*)
//    2. keyValidationStatus (RAM NSString*)
//    3. encodedcode         (RAM NSString*) must == encodestring
//    4. remainingSeconds    (RAM NSInteger on extraInfo)
//    5. savedKey            (NSUserDefaults — disk)
// ═════════════════════════════════════════════════════════════════════════════
static BOOL injectFakeSession(NSInteger seconds) {
    // Decide what key string to use:
    // Priority: existing iskey > existing savedKey > stored fake key > generate one
    NSString *keyToUse = nil;

    if (iskey && iskey.length > 0) {
        keyToUse = iskey;
    } else {
    ...
    if (!keyToUse || keyToUse.length == 0) {
        // Generate a placeholder so the disk check passes
        keyToUse = [NSString stringWithFormat:@"DEVTOOL-INJECTED-%@",
    ...
    // 3. keyValidationStatus in RAM
    keyValidationStatus = @"validated";

    // 4. encodedcode in RAM — must equal encodestring
    //    encodestring is extern const so we can read its value safely
    encodedcode = encodestring ? [encodestring copy] : @"odsugddhwxbc==";

    // 5. remainingSeconds on extraInfo singleton
    if (gExtraInfo) {
        gExtraInfo.remainingSeconds = seconds;
    } else {
        // extraInfo not captured yet — hook startUpdateTimer will set it later,
        // but we can pre-set via ivar offset once it exists.
        // Log and return partial success.
        NSLog(@"[DevTool] ⚠ extraInfo not yet captured — RAM timer not set. "
              @"Trigger a key check first, or reopen app with ForceOffline ON.");
    }

    NSLog(@"[DevTool] Session injected → key=%@  seconds=%ld  status=%@  encodedcode=%@",
          keyToUse, (long)seconds, keyValidationStatus, encodedcode);
...
// ═════════════════════════════════════════════════════════════════════════════
//  UIButton / UITapGestureRecognizer block categories
// ═════════════════════════════════════════════════════════════════════════════
...
- (void)dt_fire {
    void(^b)(void) = objc_getAssociatedObject(self, &kBtnKey);
    if (b) b();
}
@end
...
#define SEC(t)  [rows addObject:@[@"S", t]]
#define ROW(k,v)[rows addObject:@[@"R", k, v]]

    SEC(@"Package & Hash");
    ROW(@"__kHashDefaultValue",    safe(__kHashDefaultValue));
    ROW(@"__kBaseURL",             safe(__kBaseURL));
    ROW(@"effectiveDebHash",       gExtraInfo ? safe([gExtraInfo effectiveDebHash]) : @"(no session)");
    ROW(@"effectiveBaseURL",       gExtraInfo ? safe([gExtraInfo effectiveBaseURL]) : @"(no session)");
    ROW(@"Package display name",   gExtraInfo ? safe([gExtraInfo effectivePackageDisplayName]) : @"(no session)");

    SEC(@"AES Crypto");
    ROW(@"UNITOREIOS_AES_KEY",     safe(gCapturedAESKey));
    ROW(@"UNITOREIOS_AES_IV",      safe(gCapturedAESIV));
    ROW(@"kKeyEncoded (raw b64)",  safe(kKeyEncoded));
    ROW(@"kIVEncoded  (raw b64)",  safe(kIVEncoded));

    SEC(@"Integrity Tokens");
    ROW(@"encodestring  (const)",  safe(encodestring));
    ROW(@"encodedcode   (runtime)",safe(encodedcode));
    BOOL match = encodedcode && encodestring && [encodedcode isEqualToString:encodestring];
    ROW(@"tokens match?",          match ? @"YES ✓" : @"NO ✗");

    SEC(@"Session State");
    ROW(@"keyValidationStatus",    safe(keyValidationStatus));
    ROW(@"iskey (RAM)",            safe(iskey));
    ROW(@"savedKey (disk)",        safe([[NSUserDefaults standardUserDefaults] objectForKey:@"savedKey"]));
    NSInteger sec = gExtraInfo ? gExtraInfo.remainingSeconds : 0;
    ROW(@"remainingSeconds",       [NSString stringWithFormat:@"%ld  (%@)", (long)sec, fmtSec(sec)]);
    ROW(@"savedUDID",              safe([[NSUserDefaults standardUserDefaults] objectForKey:@"savedUDID"]));

    SEC(@"5-Guard Check (+paid:)");
    BOOL g1 = iskey != nil;
    BOOL g2 = [keyValidationStatus isEqualToString:@"validated"];
    BOOL g3 = match;
    BOOL g4 = iskey && [[NSUserDefaults standardUserDefaults] objectForKey:@"savedKey"] &&
              [iskey isEqualToString:[[NSUserDefaults standardUserDefaults] objectForKey:@"savedKey"]];
    BOOL g5 = sec > 0;
    ROW(@"1 iskey != nil",          g1 ? @"PASS ✓" : @"FAIL ✗");
    ROW(@"2 keyValidationStatus",   g2 ? @"PASS ✓" : @"FAIL ✗");
    ROW(@"3 encodedcode==const",    g3 ? @"PASS ✓" : @"FAIL ✗");
    ROW(@"4 iskey==savedKey",       g4 ? @"PASS ✓" : @"FAIL ✗");
    ROW(@"5 remainingSeconds > 0",  g5 ? @"PASS ✓" : @"FAIL ✗");
    ROW(@"→ +paid: result",         (g1&&g2&&g3&&g4&&g5) ? @"EXECUTE ✓" : @"BLOCK ✗");

    SEC(@"Pref Cache");
    ROW(@"PREF_CACHED_PACKAGE_NAME", safe(gCapturedPrefName));
    ROW(@"PREF_CACHED_PACKAGE_HASH", safe(gCapturedPrefHash));

    SEC(@"Dev Tool State");
    ROW(@"ForceOffline",             forceOfflineEnabled() ? @"ON" : @"OFF");
    ROW(@"extraInfo captured",       gExtraInfo ? @"YES" : @"NO");
...
#undef ROW
    ...
    // scroll
    UIScrollView *sc = [[UIScrollView alloc] initWithFrame:CGRectMake(0,45,popW,popH-45)];
    sc.alwaysBounceVertical = YES;
    [pop addSubview:sc];

    CGFloat y = 12, pad = 16;
    UIColor *sColor = [UIColor colorWithRed:0.10 green:0.78 blue:0.55 alpha:0.80];
    UIColor *kColor = [UIColor colorWithWhite:0.50 alpha:1.0];
    UIColor *vColor = [UIColor colorWithWhite:0.90 alpha:1.0];
    ...
    for (NSArray<NSString*> *r in rows) {
        if ([r[0] isEqualToString:@"S"]) {
            ...
            if ([val hasPrefix:@"PASS"] || [val hasPrefix:@"YES"] || [val hasPrefix:@"EXECUTE"])
                vl.textColor = okCol;
            else if ([val hasPrefix:@"FAIL"] || [val hasPrefix:@"NO ✗"] || [val hasPrefix:@"BLOCK"])
                vl.textColor = errCol;
            else
                vl.textColor = vColor;
    ...
    sc.contentSize = CGSizeMake(popW, y + 24);
    ...
    [UIView animateWithDuration:0.22 delay:0
        ...
        pop.transform = CGAffineTransformIdentity;
        pop.alpha = 1;
    } completion:nil];
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    // ── +7 days ───────────────────────────────────────────────────────────────
    self.addButton = [self makeBtn:@"+ Add 7 Days to Memory"
    ...
    // ── inject session ────────────────────────────────────────────────────────
    self.injectButton = [self makeBtn:@"⚡ Inject Cached Session (7d)"
    ...
    // ── dump ──────────────────────────────────────────────────────────────────
    self.dumpButton = [self makeBtn:@"⬇ Dump Memory Values"
    ...
    [tTap dt_onTap:^(id _){ [self didTapToggle]; }];
...
// ── factory helper ────────────────────────────────────────────────────────────
- (UIButton *)makeBtn:(NSString *)title frame:(CGRect)f
...
- (void)refresh {
    ...
    // inject button dims if session already healthy
    BOOL guardsFull = iskey && [keyValidationStatus isEqualToString:@"validated"]
                      && encodedcode && [encodedcode isEqualToString:encodestring] && sec > 0;
    self.injectButton.alpha = guardsFull ? 0.45 : 1.0;

    self.pillLabel.text = hasSession
        ? [NSString stringWithFormat:@"%@ %@", fOff ? @"📵" : @"📶", fmtSec(sec)]
        : [NSString stringWithFormat:@"%@ --", fOff ? @"📵" : @"📶"];
}
...
- (void)didTapAdd {
    ...
    NSLog(@"[DevTool] +7d → %ld", (long)gExtraInfo.remainingSeconds);
...
- (void)didTapInject {
    ...
    // If extraInfo wasn't captured yet, try to grab it now via KVC
    if (!gExtraInfo) {
        // Walk all known classes looking for Unitoreios instance
        // Fallback: the session disk write alone may let offline work next launch
        NSLog(@"[DevTool] Inject: extraInfo missing — disk guards set, RAM timer pending");
    }

    UIColor *flashCol = ok
        ? [UIColor colorWithRed:0.85 green:0.60 blue:0.10 alpha:0.5]
        : [UIColor colorWithRed:0.85 green:0.30 blue:0.30 alpha:0.5];
    [self flashBtn:self.injectButton col:flashCol original:[UIColor clearColor]];

    // Show brief status toast
    NSString *msg = ok ? @"Session injected ✓" : @"Inject failed ✗";
    [self showToast:msg color:ok
...
// ── micro helpers ─────────────────────────────────────────────────────────────
- (void)flashBtn:(UIButton *)btn col:(UIColor *)col original:(UIColor *)orig {
...
- (void)showToast:(NSString *)msg color:(UIColor *)col {
    ...
    t.frame = CGRectMake((self.frame.size.width - sz.width - 20)/2,
                          204, sz.width + 20, 24);
    t.alpha = 0;
    [self addSubview:t];
...
// grab extraInfo + spawn overlay after first valid session
- (void)startUpdateTimer {
    %orig;
    if (!gExtraInfo) {
        gExtraInfo = self;
        // If a session was pre-injected before extraInfo existed, apply remainingSeconds now
        NSString *savedKey = [[NSUserDefaults standardUserDefaults] objectForKey:@"savedKey"];
        if (savedKey && iskey && encodedcode && [encodedcode isEqualToString:encodestring]) {
            if (self.remainingSeconds <= 0) {
                self.remainingSeconds = SEVEN_DAYS;
                NSLog(@"[DevTool] retroactively applied remainingSeconds to captured extraInfo");
            }
        }
...
// force offline gate — checked every time Unitoreios tests connectivity
- (BOOL)isNetworkAvailable {
    if (forceOfflineEnabled()) return NO;
    return %orig;
}

// capture static AES key/IV the moment decryptAESData is first called
...
// capture pref key names after they're written by cacheResolvedPackageName
...
// ═════════════════════════════════════════════════════════════════════════════
//  CONSTRUCTOR — priority 101 fires BEFORE +load (priority 65535)
//  All %hook patches are already active by the time Unitoreios +load runs
// ═════════════════════════════════════════════════════════════════════════════
__attribute__((constructor(101)))
static void DevToolEarlyInit(void) {
    NSLog(@"[DevTool] early init (priority 101) — hooks armed before Unitoreios +load");

    // If ForceOffline is ON from a previous session, isNetworkAvailable is
    // already returning NO from this point forward — no race condition.
