...
#pragma mark - Custom Menu View Controller
...
static void showPlayerMenu() {
    ...
    vc.selectHandler = ^(NSInteger idx) {
        ...
        if (idx == 0) {
            action = ^{ applyPatchWithAlert(@"Characters", @"(<key>\\d+_c\\d+_unlock.*\\n.*)false", @"$1True"); };
        } else if (idx == 1) {
            action = ^{ applyPatchWithAlert(@"Skins", @"(<key>\\d+_c\\d+_skin\\d+.*\\n.*>)[+-]?\\d+", @"$11"); };
        } else if (idx == 2) {
            action = ^{ applyPatchWithAlert(@"Skills", @"(<key>\\d+_c_.*_skill_\\d_unlock.*\\n.*<integer>)\\d", @"$11"); };
        } else if (idx == 3) {
            action = ^{ applyPatchWithAlert(@"Pets", @"(<key>\\d+_p\\d+_unlock.*\\n.*)false", @"$1True"); };
        } else if (idx == 4) {
            action = ^{ applyPatchWithAlert(@"Level", @"(<key>\\d+_c\\d+_level+.*\\n.*>)[+-]?\\d+", @"$18"); };
        } else if (idx == 5) {
            action = ^{ applyPatchWithAlert(@"Furniture", @"(<key>\\d+_furniture+_+.*\\n.*>)[+-]?\\d+", @"$15"); };
        } else if (idx == 6) {
            action = ^{ patchGems(); };
        } else if (idx == 7) {
            action = ^{ patchRebornWithAlert(); };
        } else if (idx == 8) {
            action = ^{ patchAllExcludingGems(); };
        } else {
            [vc dismissViewControllerAnimated:YES completion:nil];
            return;
        }
        [vc dismissViewControllerAnimated:YES completion:action];
    };
...
static void showFileActionMenu(NSString *fileName) {
    ...
    vc.selectHandler = ^(NSInteger idx) {
        ...
        if (idx == 0) {
            ...
            [done addAction:[UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:nil]];
            action = ^{ [topVC() presentViewController:done animated:YES completion:nil]; };
        } else if (idx == 1) {
            ...
            [input addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]];
            action = ^{ [topVC() presentViewController:input animated:YES completion:nil]; };
        } else if (idx == 2) {
            NSString *docs = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject];
            NSString *path = [docs stringByAppendingPathComponent:fileName];
            NSError *err = nil;
...
static void showDataMenu() {
    ...
    vc.selectHandler = ^(NSInteger idx) {
        ...
        NSString *file = files[idx];
        [vc dismissViewControllerAnimated:YES completion:^{ showFileActionMenu(file); }];
    };
    UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:vc];
    [topVC() presentViewController:nav animated:YES completion:nil];
}
...
static void showMainMenu() {
    MenuViewController *vc = [[MenuViewController alloc] init];
    vc.titleStr = @"Menu";
    vc.items = @[@"Player", @"Data", @"Cancel"];
    vc.selectHandler = ^(NSInteger idx) {
        if (idx == 0) {
            [vc dismissViewControllerAnimated:YES completion:^{ showPlayerMenu(); }];
        } else if (idx == 1) {
            [vc dismissViewControllerAnimated:YES completion:^{ showDataMenu(); }];
        } else {
            [vc dismissViewControllerAnimated:YES completion:nil];
        }
    };
