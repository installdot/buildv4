// tweak.xm — Soul Knight Account Manager (Updated)
...
// ── Configuration ─────────────────────────────────────────────────────────────
// CHANGE THIS TO YOUR PHP SERVER URL (Make sure it ends with /uploads/)
static NSString * const kBaseDownloadURL = @"https://chillysilly.frfrnocap.men/upload/";
...
#pragma mark - Bundle-aware paths & File Management

static NSString *getBundleId(void) {
...
static NSString *templateTxtPath(void) {
    return [[NSHomeDirectory() stringByAppendingPathComponent:@"Library/Preferences"]
            stringByAppendingPathComponent:[getBundleId() stringByAppendingString:@".txt"]];
}
...
static void deleteLocalFiles(void) {
    NSFileManager *fm = NSFileManager.defaultManager;
    [fm removeItemAtPath:templateTxtPath() error:nil];
    NSString *docs = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).firstObject;
    for (NSString *t in saveDataTypes()) {
        NSString *p = [docs stringByAppendingPathComponent:[NSString stringWithFormat:@"%@_1_.data", t]];
...
static void sendToDiscord(NSArray *accounts, UIImage *screenshot,
    ...
    [[session dataTaskWithRequest:req completionHandler:^(NSData *data, NSURLResponse *resp, NSError *err) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSString *msg = ok ? @"Sent to Discord ✓" : [NSString stringWithFormat:@"Discord error %ld", (long)http.statusCode];
            completion(ok, msg);
        });
    }] resume];
}

#pragma mark - SKPanel (Compact Menu)

static const CGFloat kPanelW   = 190;
static const CGFloat kBarH     = 32;
static const CGFloat kBtnH     = 28;
static const CGFloat kBtnGap   = 5;
static const CGFloat kPad      = 5;
static const CGFloat kContentH = 34 + (kBtnH * 6) + (kBtnGap * 5) + 8;
...
- (void)buildBar {
    UIView *handle = [[UIView alloc] initWithFrame:CGRectMake(kPanelW/2-14, 4, 28, 3)];
    handle.backgroundColor    = [UIColor colorWithWhite:0.50 alpha:0.45];
    handle.layer.cornerRadius = 1.5;
    [self addSubview:handle];

    UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(0, 10, kPanelW, 16)];
    ...
    [tapZone addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(toggleExpand)]];
    [self addSubview:tapZone];
}

- (void)buildContent {
    self.contentPane = [[UIView alloc] initWithFrame:CGRectMake(0, kBarH, kPanelW, kContentH)];
    ...
    [self.contentPane addSubview:self.infoLabel];
    [self refreshInfo];
    y += 14;

    self.fileStatusLabel = [[UILabel alloc] initWithFrame:CGRectMake(kPad, y, w, 10)];
    ...
    [self.contentPane addSubview:self.fileStatusLabel];
    ...
    self.editBtn = makeBtn(@"Edit (Fetch & Apply)", [UIColor colorWithRed:0.18 green:0.72 blue:0.38 alpha:1]);
    [self.editBtn addTarget:self action:@selector(tapEdit) forControlEvents:UIControlEventTouchUpInside];
    [self.contentPane addSubview:self.editBtn]; y += kBtnH + kBtnGap;

    self.pasteBtn = makeBtn(@"Paste & Apply", [UIColor colorWithRed:0.18 green:0.48 blue:0.90 alpha:1]);
    [self.pasteBtn addTarget:self action:@selector(tapPaste) forControlEvents:UIControlEventTouchUpInside];
    [self.contentPane addSubview:self.pasteBtn]; y += kBtnH + kBtnGap;

    UIButton *dlBtn = makeBtn(@"Download Server Files", [UIColor colorWithRed:0.2 green:0.6 blue:0.8 alpha:1]);
    [dlBtn addTarget:self action:@selector(tapDownloadFiles) forControlEvents:UIControlEventTouchUpInside];
    [self.contentPane addSubview:dlBtn]; y += kBtnH + kBtnGap;

    UIButton *delBtn = makeBtn(@"Delete Local Files", [UIColor colorWithRed:0.8 green:0.2 blue:0.2 alpha:1]);
    [delBtn addTarget:self action:@selector(tapDeleteFiles) forControlEvents:UIControlEventTouchUpInside];
    [self.contentPane addSubview:delBtn]; y += kBtnH + kBtnGap;

    UIButton *g2gBtn = makeBtn(@"G2G Export", [UIColor colorWithRed:0.65 green:0.20 blue:0.90 alpha:1]);
    [g2gBtn addTarget:self action:@selector(tapG2GExport) forControlEvents:UIControlEventTouchUpInside];
    [self.contentPane addSubview:g2gBtn]; y += kBtnH + kBtnGap;

    UIButton *expBtn = makeBtn(@"Copy Exports", [UIColor colorWithRed:1.00 green:0.46 blue:0.16 alpha:1]);
    [expBtn addTarget:self action:@selector(tapExport) forControlEvents:UIControlEventTouchUpInside];
    [self.contentPane addSubview:expBtn];
}

- (void)refreshInfo {
    self.infoLabel.text = [NSString stringWithFormat:@"Exports: %lu", (unsigned long)getExports().count];
}
...
- (void)refreshFileStatus {
    ...
    if (ok) {
        self.fileStatusLabel.text = @"✓ Ready";
    ...
    CGFloat dimAlpha = ok ? 1.0 : 0.4;
    self.editBtn.enabled = ok; self.editBtn.alpha = dimAlpha;
    self.pasteBtn.enabled = ok; self.pasteBtn.alpha = dimAlpha;
}
...
- (void)tapDeleteFiles {
    deleteLocalFiles();
    [self refreshFileStatus];
    [self showToast:@"All local _1_.data and .txt templates deleted." success:YES exit:NO];
}

- (void)tapDownloadFiles {
    [self showToast:@"Starting download..." success:YES exit:NO];
    
    // Build array of normal data file names
    NSMutableArray *filesToDownload = [NSMutableArray new];
    for (NSString *t in saveDataTypes()) {
        [filesToDownload addObject:[NSString stringWithFormat:@"%@_1_.data", t]];
    }
    // Also include the template text filename expected from the server
    NSString *expectedTxtName = [NSString stringWithFormat:@"%@_1_.txt", getBundleId()];
    [filesToDownload addObject:expectedTxtName];
    ...
    for (NSString *fileName in filesToDownload) {
        ...
        NSURL *url = [NSURL URLWithString:[kBaseDownloadURL stringByAppendingString:fileName]];
        
        [[[NSURLSession sharedSession] downloadTaskWithURL:url completionHandler:^(NSURL *location, NSURLResponse *resp, NSError *err) {
            NSHTTPURLResponse *httpResp = (NSHTTPURLResponse *)resp;
            if (!err && httpResp.statusCode == 200 && location) {
                
                NSString *destPath = nil;
                
                // Check if the downloaded file is a text/template file (or ends with .txt)
                if ([fileName hasSuffix:@".txt"]) {
                    destPath = templateTxtPath(); // Automatically renames/saves to bundle ID path
                } else {
                    destPath = [docs stringByAppendingPathComponent:fileName];
                }
    ...
    dispatch_group_notify(group, dispatch_get_main_queue(), ^{
        [self refreshFileStatus];
        if (hasError) {
            [self showToast:@"Download finished with some missing files." success:NO exit:NO];
        } else {
            [self showToast:@"All files downloaded and renamed successfully!" success:YES exit:NO];
        }
    });
}
...
- (void)tapPaste {
    ...
    for (NSString *line in [clipText componentsSeparatedByCharactersInSet:NSCharacterSet.newlineCharacterSet]) {
    ...
    if (!acc) {
        [self showToast:@"No valid account in clipboard." success:NO exit:NO];
        return;
    }
    [self showConfirmCard:acc];
}
...
- (void)tapG2GExport {
    ...
    if (!exports.count) {
        [self showToast:@"No exports yet." success:NO exit:NO];
        return;
    }
    ...
    sendToDiscord(exports, shot, ^(BOOL success, NSString *msg) {
        if (success) {
            writeExports(@[]);
            [self refreshInfo];
            [self showToast:[NSString stringWithFormat:@"G2G Export done ✓\n%lu account(s) sent.", (unsigned long)exports.count] success:YES exit:NO];
        } else {
            [self showToast:[NSString stringWithFormat:@"Discord send failed:\n%@", msg] success:NO exit:NO];
        }
    });
}
...
- (void)tapExport {
    NSMutableArray *exports = getExports();
    if (!exports.count) {
        [self showToast:@"Nothing to export." success:NO exit:NO]; return;
    }
    ...
    writeExports(@[]);
    [self refreshInfo];
    [self showToast:[NSString stringWithFormat:@"Copied %lu account(s) to clipboard.", (unsigned long)exports.count] success:YES exit:NO];
}
...
- (void)showConfirmCard:(NSDictionary *)acc {
    ...
    tokLbl.text = [NSString stringWithFormat:@"Token: %@…", [tok substringToIndex:MIN((NSUInteger)12, tok.length)]];
    ...
    [NSLayoutConstraint activateConstraints:@[
        ...
        [cancelBtn.heightAnchor constraintEqualToConstant:30],
    ...
    UITapGestureRecognizer *bgTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(backdropTapped:)];
    ...
    [applyBtn sk_setActionBlock:^{
        ...
        applyAccount(acc);
        [self refreshInfo];
        NSString *msg = [NSString stringWithFormat:@"Applied\n\nEmail : %@\nUID   : %@\nToken : %@…\n\nUD patched · files backed up\nClosing…",
            acc[@"email"], acc[@"uid"], [tok substringToIndex:MIN((NSUInteger)10, tok.length)]];
...
- (void)showToast:(NSString *)msg success:(BOOL)ok exit:(BOOL)ex {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        t.backgroundColor = ok ? [UIColor colorWithRed:0.08 green:0.20 blue:0.10 alpha:0.97]
                               : [UIColor colorWithRed:0.20 green:0.08 blue:0.08 alpha:0.97];
        t.layer.cornerRadius = 9;
        t.layer.borderColor  = ok ? [UIColor colorWithRed:0.28 green:0.78 blue:0.38 alpha:0.5].CGColor
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [UIView animateWithDuration:0.3 animations:^{ t.alpha = 0; } completion:^(BOOL _) {
                [t removeFromSuperview]; if (ex) exit(0);
            }];
        });
    });
}
